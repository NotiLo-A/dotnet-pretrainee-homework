...
public class AuthorsController : ControllerBase
{
    ...
    {
        if (string.IsNullOrWhiteSpace(author.Name)) return BadRequest(new { message = "Author name is required" });
    ...
    public IActionResult UpdateAuthor(int id, Author author)
    {
        ...
        if (existingAuthor == null) return NotFound(new { message = $"Author with ID {id} not found" });
    ...
    public IActionResult DeleteAuthor(int id)
    {
        ...
        if (hasBooks) return BadRequest(new { message = "Cannot delete author who has books" });
